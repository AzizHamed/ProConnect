...
For the server to work, you have to add the Config Vars in the project settings, namely:
- `GOOGLE_CREDENTIALS`, which contains the settings required to connect to the Firebase Admin SDK.
- `PROJECT_PATH`, which contains the name of the folder containing the backend project "ProConnect_Backend". Without this, the builds would fail.

In addition, the following buildpacks are needed, in this order:
